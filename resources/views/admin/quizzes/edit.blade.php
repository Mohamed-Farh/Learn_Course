                    ...
                    <div class="card-header bg-white border-0">
                        <div class="row align-items-center">
                            ...
                            <div class="col-4 text-right">
                                <a href="{{ route('quizzez.index') }}" class="btn btn-lg btn-primary">{{ __('Back to list') }}</a>
                            </div>
                        </div>
                    </div>
                    <div class="card-body">
                        <form method="post" action="{{ route('quizzez.update', $quiz) }}" autocomplete="off">
                            @csrf
                            @method('PATCH')
                                        ...
                                        <option 
                                            <?php
                                                if($quiz->course->id == $course->id)
                                                    echo 'selected'
                                             ?>
                                        value="{{ $course->id }}"> {{ \Str::limit($course->title, 30)}} </option>
                                        @endforeach
                                    </select>
